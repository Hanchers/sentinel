...
function openToCurrentTab(obj) {
    if (isScrollToTop) {
        $(obj).show().siblings('.sentinel_iframe').hide();
    } else {
        $(obj).css({"visibility": "visible", "position": "static"}).siblings('.sentinel_iframe').css({"visibility": "hidden", "position": "absolute", "left": "0", "top": "0"});
    }
}
...
$(function() {
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .sentinel_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            openToCurrentTab(this);
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="sentinel_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" data-id="' + dataUrl + '" data-refresh="' + isRefresh + '" ></iframe>';
            if (isScrollToTop) {
                $('.mainContent').find('iframe.sentinel_iframe').hide();
            } else {
                $('.mainContent').find('iframe.sentinel_iframe').css({"visibility": "hidden", "position": "absolute", "left": "0", "top": "0"});
            }
...
//            });
    ...
    function refreshTab() {
    	...
    	var target = $('.sentinel_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.sentinel_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            if (isScrollToTop) {
                $('.sentinel_iframe[data-id="' + $(this).data('id') + '"]').show();
            } else {
                $('.sentinel_iframe[data-id="' + $(this).data('id') + '"]').css({"visibility": "visible", "position": "static"});
            }
